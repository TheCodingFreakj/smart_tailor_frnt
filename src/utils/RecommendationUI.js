  ...
  return (
    ...
    <Box sx={{ padding: '24px' }}>
      ...
      <Box sx={{ padding: '24px' }}>
       ...
       {selectedArea == 'area1' ? <>{/* Select Customer */}
      <FormControl fullWidth sx={{ marginBottom: '16px' }}>
      ...
      <FormControl fullWidth sx={{ marginBottom: '16px' }}>
        ...
        <Select
          value={"jkjhb"}
          onChange={(e)=> handleSnippetSelect(e)}
          label="Choose Code Snippet"
        >
          {codeSnippets.map((snippet) => {
            return <MenuItem key={snippet.id} value={snippet.id}>
            {snippet.title}
          </MenuItem>
          })}
        </Select>
      ...
      {jsonHolder.length > 0 && <CardEditor comp={jsonHolder}/>}
